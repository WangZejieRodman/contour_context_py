...
class ContourManager:
    ...
    def _make_contours_interval_based(self, cc_roi: Tuple[int, int, int, int]):
        ...
        将高度分为不同区间：
        - L0: [lv_grads[0], lv_grads[1]]
        - L1: [lv_grads[1], lv_grads[2]]
        - L2: [lv_grads[2], lv_grads[3]]
        - ...
        - L(n-1): [lv_grads[n-1], +inf]
        """
        x, y, w, h = cc_roi
        ...
        for level in range(len(lv_grads)):
            print(f"[INTERVAL_DEBUG] Processing level {level}")

            # 确定当前层级的高度区间
            if level == 0:
                # 第一层：从最小高度到第一个阈值
                h_min = self.min_bin_val  # 使用实际的最小高度
                h_max = lv_grads[0]
            ...
            # 创建区间掩码
            if h_max == float('inf'):
                interval_mask = (bev_roi >= h_min).astype(np.uint8) * 255
            else:
